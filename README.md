...
边缘函数部署的**多盘网盘聚合工具**。把对象存储（S3 兼容：S3 / R2 / OSS / COS / MinIO）与 OneDrive（组织租户证书鉴权）两类"盘"聚合成一个统一的浏览界面，下载走 **302 直链**（浏览器原生多线程、不耗边缘 CPU）。
...
- **隐藏目录**：通过管理页面或 xlsx 配置隐藏目录/文件（仅界面隐藏，硬路径仍可访问）。
...
- 文件管理：管理员登录后支持文本编辑、移动/重命名、删除、创建目录（需存储后端支持）。
- 文件夹密码是访问门禁，不是文件字节加密。
